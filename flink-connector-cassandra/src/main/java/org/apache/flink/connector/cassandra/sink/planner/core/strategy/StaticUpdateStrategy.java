...
/**
 * Static strategy for UPDATE queries that returns the user-provided query string exactly as
 * configured.
 *
 * <p>This strategy is used when users provide their own UPDATE query string.
 *
 ...
 */
@Internal
public final class StaticUpdateStrategy<T> implements PlannerStrategy<T> {
    ...
    public QueryWithBindings getQueryWithBindings(
        ...
        Preconditions.checkArgument(write.isUpdate(), "StaticUpdateStrategy got INSERT: " + write);
    ...
    public Object[] getOrderedBindValues(ResolvedWrite write, ClauseBindings ignored) {
        ...
        Object[] out = new Object[set.length + where.length];
        System.arraycopy(set, 0, out, 0, set.length);
        System.arraycopy(where, 0, out, set.length, where.length);
        return out;
    }
}
